...
import { FbLike, FbUser } from '../../types';
...
export class LikesPage implements OnInit {
  ...
  readonly filteredLikes = computed(() => {
    const q = this.filter().toLowerCase();
    if (!q) return this.likes();
    return this.likes().filter((l) => l.name?.toLowerCase().includes(q));
  });

  readonly redirectUri = `${window.location.origin}/likes`;
  ...
  loadLikes(after?: string): void {
    const token = this.auth.token();
    if (!token || this.loading()) return;
    this.loading.set(true);

    this.api.getLikes(token, 100, after).subscribe({
      next: (res) => {
        this.likes.update((prev) => (after ? [...prev, ...res.data] : res.data));
  ...
  loadMore(): void {
    if (this.hasMore() && this.nextCursor()) {
      this.loadLikes(this.nextCursor());
    }
  }
